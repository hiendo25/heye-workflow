...
SHOTS = "C:/Users/dotha/AppData/Local/Temp/chart-research/pick"
OUT = "D:/hiendo25/heye-workflow/docs/HeyE-Nghien-cuu-bieu-do-thong-ke.docx"
...
def field(name, desc, required=False, num=None):
    ...
    r.bold = True
    r.font.size = Pt(10)
    par.add_run(" — ").font.size = Pt(10)
    # Tách theo cặp ** để in đậm cụm nằm giữa câu
    for j, seg in enumerate(desc.split("**")):
        if not seg:
            continue
        rr = par.add_run(seg)
        rr.font.size = Pt(10)
